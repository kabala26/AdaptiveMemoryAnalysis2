...
import { Users, ShieldAlert, HardDrive, Cpu, FileSearch, Loader2, AlertTriangle, Trash2, Download, TrendingDown } from 'lucide-react'
...
function StatCard({ label, value, sub, icon: Icon, accent = 'text-navy-700 dark:text-navy-300', iconBg = 'bg-navy-100 dark:bg-navy-800/40' }) {
  return (
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(() => {
    Promise.all([
      ...
      api.get('/analysis/drift').catch(() => ({ data: null })),
    ]).then(([s, d, dr]) => {
      setStats(s.data)
  ...
  const columns = useMemo(() => [
    ...
    { id: 'view',     header: '', cell: i => <Link to={`/results/${i.row.original.dump_id}`} className="text-xs text-navy-700 dark:text-navy-300 hover:underline font-medium">View →</Link> },
  ...
  return (
    <Layout>
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Admin Dashboard</h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            {/* Confidence drift card */}
            {drift?.status === 'ok' ? (
              <div className={`bg-white dark:bg-gray-800 rounded-2xl border-2 p-6 shadow-sm ${
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    drift.drift_detected ? 'bg-red-100 dark:bg-red-900/20' : 'bg-green-100 dark:bg-green-900/20'
                  }`}>
                    <TrendingDown className={`w-5 h-5 ${drift.drift_detected ? 'text-red-500' : 'text-green-500'}`} />
                  </div>
                </div>
                <p className="text-[10px] font-mono text-gray-400 uppercase tracking-widest">Confidence Drift</p>
                <p className={`text-3xl font-bold font-mono mt-1 ${drift.drift_detected ? 'text-red-500' : 'text-green-500'}`}>
                  {drift.drift_detected ? 'DETECTED' : 'STABLE'}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  {drift.drift_detected
                    ? `Baseline ${(drift.baseline_mean * 100).toFixed(1)}% → Recent ${(drift.recent_mean * 100).toFixed(1)}% (p=${drift.p_value})`
                    : `Baseline ${(drift.baseline_mean * 100).toFixed(1)}% vs Recent ${(drift.recent_mean * 100).toFixed(1)}% — no significant shift`
                  }
                </p>
              </div>
            ) : drift?.status === 'insufficient_data' ? (
              <StatCard label="Confidence Drift" value="Not enough data" icon={TrendingDown}
                sub={`Need ${drift.min_required} predictions (have ${drift.total})`}
                accent="text-gray-400" iconBg="bg-gray-100 dark:bg-gray-700" />
            ) : null}
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <Link key={l.href} to={l.href} className="px-4 py-2 text-xs font-medium rounded-lg border border-navy-300 dark:border-navy-700 text-navy-700 dark:text-navy-300 hover:bg-navy-50 dark:hover:bg-navy-900/20 transition-colors">
              {l.label}
            </Link>
          ))}
        </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {table.getRowModel().rows.map(row => (
                    <tr key={row.id} className="hover:bg-navy-50/30 dark:hover:bg-navy-900/10 transition-colors">
